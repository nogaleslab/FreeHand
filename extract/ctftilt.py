...
for file in list: 
	...
	if os.path.isfile(file) & os.path.isfile(tiltname):
		...
		fname = file.strip('.mrc')
			...
			p1.write('%s		%s		%s		%s	%s	%s	%s\n' %(out3[0],out3[1],out3[2],out3[3],out3[4],str(particle1),str(micro)))
		...
		a = subprocess.Popen(cmd, -1, stdin=subprocess.PIPE,stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                ...
                while i <= tot:
                        ...
                        p2.write('%s    	%s      	%s      	%s	%s	%s	%s\n' %(out3[0],out3[1],out3[2],out3[3],out3[4],str(particle2),str(micro)))
