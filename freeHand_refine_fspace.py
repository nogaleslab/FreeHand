...
def setupParserOptions():
        ...
        parser.set_usage("%prog -t <tilted stack> -m <model> -p <parameter file> -c <ctf info>")
        ...
        parser.add_option("-c",dest="ctf1",type="string", metavar="FILE",
                help="Output parameter file from refine_fspace for UNTILTED particles")
        parser.add_option("-e",dest="ctf2",type="string", metavar="FILE",
                help="Per-particle CTF-information for TILTED particles")
...
def main(params):
	...
	cmd = '%s/fspace_param_consolidate.py %s %s' %(cwd,ctf1,ctf2)
	if debug is True:
		print cmd
